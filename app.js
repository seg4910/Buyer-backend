...
router.post('/purchaseService', function (req, res) {
  ...
  var taskSize = req.body.taskSize;
  ...
  if (serviceId === undefined || userId === undefined) {
    res.status(404).send();
    throw err;
  };
  ...
  stripe.charges.create({
    ...
    var sql = `INSERT INTO orders (sellerId, buyerId, serviceId, dateOrdered, price, serviceCategory, sellerName, dateScheduled, status, serviceName, size, note, shiftTime) 
    VALUES ('${sellerId}', '${userId}', '${serviceId}', '${date}', '${price}', '${serviceCategory}', '${sellerName}', '${selectedDay}', '${status}', '${serviceName}', '${taskSize}', '${noteToSeller}', '${selectedTime}')`;
...
router.post('/newCardStripe', function (req, res) {
  ...
  if (cusID === undefined || token === undefined) {
    res.status(404).send();
    throw err;
  };
...
router.get('/createAccount', function (req, res) {
  ...
  // HANDLE ERROR
  if (email === undefined || name === undefined || password === undefined || phone === undefined || type === undefined) {
    res.status(404).send();
    throw err;
  };
